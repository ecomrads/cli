...
export function registerAuthCommands(program: Command, globals: GlobalOptions): void {
  ...
  auth
    ...
    .action(() => {
      throw new Error(
        "browser OAuth is not wired yet — use:\n  ecomrads auth token <access-token>\n\nUse the same Supabase JWT the MCP server forwards to FastAPI (Authorization: Bearer).",
      );
    });

  auth
    .command("token")
    .description("Save a Supabase access token (Bearer JWT)")
  ...
  auth
    .command("imgbb-key")
    .description("Save ImgBB API key for upload")
    ...
    .action(async () => {
      ...
      if (globals.json) {
        printJson({
          ...
          upload_configured: isUploadConfigured(cfg),
      ...
      console.log(
        isUploadConfigured(cfg)
          ? "Upload: IMGBB_API_KEY configured"
          : "Upload: IMGBB_API_KEY missing",
      );
    });
...
export function registerUploadCommand(program: Command, globals: GlobalOptions): void {
  program
    .command("upload")
    .description("Upload an image via ImgBB and get a public URL")
    ...
    .action(async (target: string, opts: { expiration: string }) => {
      const cfg = await loadConfig();
      if (!isUploadConfigured(cfg)) {
        throw new Error(
          "IMGBB_API_KEY required — set env or run: ecomrads auth imgbb-key <key>",
        );
      }
      const { uploadImage, formatUploadHuman } = await import("../upload/imgbb.js");
...
export function registerVersionCommand(program: Command, globals: GlobalOptions): void {
  const VERSION = "0.1.0";
  const COMMIT = process.env.ECOMRADS_CLI_COMMIT ?? "dev";
